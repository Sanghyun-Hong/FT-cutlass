...
def training_log(
    ...
    # print(f"loss: {avg}, grad norm: {grad_norm}")
    return report_memory_flag, avg, grad_norm
...
def checkpoint_and_decide_exit(
    ...
    local_should_exit = 0
    target_iteration = 500
    exit_message = f"Global exit signal received at iteration {iteration}"
    job_id = os.getenv('SLURM_JOB_ID')

    global _GLOBAL_EMA_LOSS, _EMA_ALPHA
    local_ema_val = 0.0
    has_valid_loss = 0

    if step_loss is not None:
        current_loss_val = step_loss.item() if torch.is_tensor(step_loss) else step_loss
        # caculate loss EMA
        if _GLOBAL_EMA_LOSS is None:
            _GLOBAL_EMA_LOSS = current_loss_val
        else:
            _GLOBAL_EMA_LOSS = _EMA_ALPHA * current_loss_val + (1 - _EMA_ALPHA) * _GLOBAL_EMA_LOSS
            
        local_ema_val = _GLOBAL_EMA_LOSS
        has_valid_loss = 1
    
    ema_comm_tensor = torch.tensor([local_ema_val, float(has_valid_loss)], 
                               dtype=torch.float32, 
                               device=torch.cuda.current_device())
    torch.distributed.all_reduce(ema_comm_tensor, op=torch.distributed.ReduceOp.MAX)
    # 更新所有 Rank 的全局状态
    synced_ema = ema_comm_tensor[0].item()
    is_valid_update = ema_comm_tensor[1].item() > 0
    
    if is_valid_update:
        _GLOBAL_EMA_LOSS = synced_ema

    if perform_FI:
        if step_grad_norm is not None:
            val = step_grad_norm.item() if torch.is_tensor(step_grad_norm) else step_grad_norm
            if math.isnan(val):
                local_should_exit = 1
                exit_message = f"detect NAN grad norm, exiting program at iteration {iteration}"

        # if step_loss is not None:
        #     target_loss = 1.5
        #     current_loss_val = step_loss.item() if torch.is_tensor(step_loss) else step_loss

        #     if current_loss_val <= target_loss and iteration >= target_iteration:
        #         local_should_exit = 1
        #         exit_message = f'Back to target loss after target iteration, exiting program at iteration {iteration}'

        if is_valid_update:
            # Qwen3: 0.8916453719139099; Qwen2.5: 1.0468945503234863; Llama3: 1.2381027936935425
        ...
        if iteration >= target_iteration:
            local_should_exit = 1
            exit_message = f"Reach to target iteration, exiting program at iteration at {iteration}, EMA Loss ({synced_ema})"
        
    exit_flag = torch.tensor(local_should_exit, dtype=torch.int, device=torch.cuda.current_device())
    torch.distributed.all_reduce(exit_flag, op=torch.distributed.ReduceOp.MAX)
    ...
    if exit_flag.item() > 0:
        ...
        torch.distributed.barrier()
        # print_datetime(exit_message)
        if torch.distributed.get_rank() == 0 or local_should_exit == 1:
            print_datetime(exit_message)
    ...
    # # if grad_norm is NAN, exit
    # if (step_grad_norm == float('nan')):
    #     if args.save:
    #         save_checkpoint_and_time(
    #             iteration,
    #             model,
    ...
    #         )
    #     torch.distributed.barrier()
    #     print_datetime(f'detect NAN grad norm, exiting program at iteration {iteration}')

    #     return True
    
    # FI and if loss back to target loss after target iteration
    # local_should_exit = 0
    # if step_loss is not None and perform_FI:
    #     target_loss = 1.5
    #     target_iteration = 750
    #     current_loss_val = step_loss.item() if torch.is_tensor(step_loss) else step_loss
    ...
    #     torch.distributed.barrier()
    #     print_datetime(f'Back to target loss after target iteration, exiting program at iteration {iteration}')

    #     return True

    # if (step_loss != None and perform_FI == True):
    #     target_loss = 1.5
    #     target_iteration = 750
    #     if (step_loss < target_loss and iteration > (target_iteration-1)):
    #         if args.save:
    #             save_checkpoint_and_time(
    #                 iteration,
    #                 model,
...
def train(
    ...
    # Core-Checker Control and Fault Injection Setting
    job_id = os.getenv('SLURM_JOB_ID')
    gpu = torch.cuda.current_device()
    
    controlFP = f"./control_{job_id}/{gpu}/perform.txt"
    FIFP = f"./control_{job_id}/{gpu}/FI.txt"
    PlanFP = f"./control_{job_id}/{gpu}/plan.txt"
    falutyStepFP = f"./control_{job_id}/{gpu}/faulty_step.txt"
    totalfaultyStepFP = f"./control_{job_id}/{gpu}/total_faulty_steps.txt"
    CurStepFP = f"./control_{job_id}/{gpu}/current_step.txt"
    faultyGPUFP = f"./control_{job_id}/faulty_GPU.txt"
    ...
    with open(faultyGPUFP, 'r') as file:
        faulty_GPU = int(file.readline())
    
    total_FI_iter = len(faulty_steps_list) * faulty_duration
    
    # print(f"current gpu: {gpu}, faulty step begin: {faulty_step}, faulty step end: {total_fi_steps}, faulty gpu: {faulty_GPU}")
    faulty_steps_count = 0
    perform_FI = False
    if gpu == faulty_GPU:
    ...
    while iteration < args.train_iters:
        ...
        if perform_FI:
            ...
            with open(CurStepFP, "w") as file:
                file.truncate(0)
                file.write(f"{iteration} ")
            if(iteration == faulty_step):
                with open(FIFP, "w") as file:
                    file.truncate(0)
                    file.write('t')
            elif (iteration > faulty_step and iteration < (total_fi_steps)):
                # delete one line from injection plan based one step
                if(((iteration + 1) % FI_step == 1) or FI_step == 1) :
                    with open(PlanFP, "r") as file:
                        lines = file.readlines()
                    lines.pop(0)
                    with open(PlanFP, "w") as file:
                        file.writelines(lines)
                    faulty_steps_count += 1
        ...
        # Core-Checker: Record Faults Behaviours Control
        if faulty_GPU != -1:
            if iteration >= faulty_step and iteration < total_fi_steps:
                # write current global step
                with open(CurStepFP, "w") as file:
                    file.truncate(0)
                    file.write(f"{iteration} ")
        ...
        report_memory_flag, step_loss, step_grad_norm = training_log(
            loss_dict,
            total_loss_dict,
            learning_rate,
            decoupled_learning_rate,
...
def evaluate(
    ...
    """Evaluation."""
    # Core-Checker: Disable cutlass before evaluating
    job_id = os.getenv('SLURM_JOB_ID')
    gpu = torch.cuda.current_device()
    controlFP = f"./control_{job_id}/{gpu}/perform.txt"
